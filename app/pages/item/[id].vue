...
<template>
  <div>
    <UPageSection v-if="itemData">
      ...
      <h1 class="text-4xl font-bold">{{ itemData.descriptionId }}</h1>
      <div class="grid sm:grid-cols-2 lg:grid-cols-3">
        <UCard class="m-2">
          <UIcon name="bi:geo-alt-fill" class="size-8"/>
          <h2 class="text-lg font-semibold">アイテムID</h2>
          <p>{{ itemId.full }}</p>
        </UCard>
        <UCard class="m-2">
          <UIcon name="bi:chat-square-text-fill" class="size-8"/>
          <h2 class="text-lg font-semibold">翻訳キー</h2>
        ...
        <UCard class="m-2">
          ...
          <p class="wrap-break-word">{{ itemData.class }}</p>
        </UCard>
      </div>
    </UPageSection>
  </div>
