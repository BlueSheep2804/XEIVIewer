        ...
        <div class="mt-2 p-4 sm:w-lg bg-elevated rounded-lg grid gap-4 grid-cols-1 sm:grid-cols-2">
          <template v-for="(entry, index) in entries">
            <UFormField :label="entry.label">
              <UInputMenu v-if="entry.items.length" :items="entry.items" @change="updateUrlQuery" v-model="search[index]" clear>

              </UInputMenu>
              <UInput v-else @change="updateUrlQuery" v-model="search[index]">
                <template #trailing v-if="search[index]?.length">
                  <UButton
                    icon="bi:x-circle"
                    variant="link"
                    color="neutral"
